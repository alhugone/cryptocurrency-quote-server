...
namespace CoinbasePro.Shared
{
    public class StringEnumWithDefaultConverter : JsonConverter
    {
            ...
            JsonSerializer serializer)
        {
            ...
            var unknownName =
                names.SingleOrDefault(n => string.Equals(n, UnknownValue, StringComparison.OrdinalIgnoreCase));
            if (unknownName == null)
        ...
        {
            if (FromValueMap == null) FromValueMap = new Dictionary<Type, Dictionary<string, object>>();
            if (ToValueMap == null) ToValueMap = new Dictionary<Type, Dictionary<object, string>>();
            if (FromValueMap.ContainsKey(enumType)) return;
